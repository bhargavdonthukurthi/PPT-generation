...
def _update_table(table: Any, new_planned_user_stories: Dict, rally_report_generator: RallyReportGenerator, teams: List[str]) -> None:
    """
    Updates table content and dynamically removes rows for teams without data.
    """
    # Deleting rows for teams that are selected but have no data
    if len(table.rows[0].cells) > 1 and table.rows[0].cells[0].text.strip() == "Application":
        rows_to_delete = []
        for i, row in enumerate(table.rows):
            if i == 0:  # Skip header row
                continue
            app_name = row.cells[0].text.strip()
            # Check if this team has any planned or mid-sprint stories
            has_planned_stories = app_name in new_planned_user_stories.get('iteration_data', {}) and new_planned_user_stories['iteration_data'][app_name]
            mid_sprint_key = f"{app_name}-midsprint"
            has_mid_sprint_stories = mid_sprint_key in new_planned_user_stories.get('iteration_data', {}) and new_planned_user_stories['iteration_data'][mid_sprint_key]

            if app_name and not (has_planned_stories or has_mid_sprint_stories):
...
def update_ppt_with_data(ppt_file: str, data: Dict, output_file: str, rally_report_generator: RallyReportGenerator, start_date, end_date: str = None, teams=[]) -> bool:
    try:
        ...
        if not end_date:
            slides_to_remove_indices = get_slides_to_remove_dynamically(prs, teams)
            remove_slides(prs, slides_to_remove_indices)
     ...
     # Determine which items go where
        ...
        for slide in prs.slides:
            for shape in slide.shapes:
                if shape.has_table:
                    _update_table(shape.table, data, rally_report_generator, teams)
...
def get_slides_to_remove_dynamically(prs: Presentation, selected_teams: List[str]) -> List[int]:
    """
    Scans the presentation to identify which slides to remove by checking both slide
    titles and table contents for team keywords.
    """
    slide_indices_to_remove = []

    # Comprehensive mapping of project names to all possible keywords
    team_to_title_keywords = {
        "ADB": ["EMIS", "CBIP", "D&B", "EMIS UI", "EMIS - Backend"],
        "RDM": ["RDM"],
        "Data BAU": ["KPI", "EDW", "Trade Credit"],
        "SCUP NA Datamart - KPI": ["SCUP NA"],
        "CDL": ["CDL"],
        "CDH": ["CDH"],
    }

    # Get a flat list of all possible team keywords for scanning
    all_team_keywords = [keyword for sublist in team_to_title_keywords.values() for keyword in sublist]

    # Get the specific keywords for the teams the user selected
    keywords_to_keep = []
    for team in selected_teams:
        keywords_to_keep.extend(team_to_title_keywords.get(team, []))
    
    # Ensure combined slide for CDL/CDH is kept if either is selected
    if "CDH" in keywords_to_keep or "CDL" in keywords_to_keep:
        keywords_to_keep.extend(team_to_title_keywords.get("CDL", []))
        keywords_to_keep.extend(team_to_title_keywords.get("CDH", []))
    keywords_to_keep = list(set(keywords_to_keep))


    for i, slide in enumerate(prs.slides):
        # Always keep the first (title) and last (placeholder) slides.
        # Also, keep any slide with "Milestone" in the title.
        if i == 0 or (slide.shapes.title and "Milestone" in slide.shapes.title.text):
            continue

        teams_found_on_slide = set()

        # 1. Check the slide title for any known team keyword
        if slide.shapes.title and slide.shapes.title.has_text_frame:
            title_text = slide.shapes.title.text
            for keyword in all_team_keywords:
                if keyword in title_text:
                    teams_found_on_slide.add(keyword)

        # 2. Check the first cell of each row in any table for team names
        for shape in slide.shapes:
            if shape.has_table:
                table = shape.table
                if len(table.rows) > 0 and len(table.rows[0].cells) > 0:
                     # Check the "Application" column for team names
                    for row in table.rows:
                        app_name = row.cells[0].text.strip()
                        if app_name in all_team_keywords:
                            teams_found_on_slide.add(app_name)
        
        # If no teams could be identified on the slide, remove it by default
        if not teams_found_on_slide:
            slide_indices_to_remove.append(i)
            continue
            
        # 3. Decide whether to keep or remove the slide
        # Keep the slide if there's any overlap between teams found and teams to keep
        if not any(team in keywords_to_keep for team in teams_found_on_slide):
            slide_indices_to_remove.append(i)

    return slide_indices_to_remove

def remove_slides(ppt, slides_indices):
    """
    Removes slides from a presentation based on a list of indices.
    """
